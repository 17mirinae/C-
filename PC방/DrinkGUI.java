...
public class DrinkGUI extends JFrame {
	...
	public DrinkGUI() {
		...
		try {
			...
			String query = "SELECT * FROM DRINK ORDER BY DRINK_ID";

			ResultSet rs = stmt.executeQuery(query);

			while (rs.next()) {
				int menu_id = rs.getInt("DRINK_ID");
				String menu_name = rs.getString("DRINK_NAME");
				int menu_price = rs.getInt("DRINK_PRICE");
				String menu_img = rs.getString("DRINK_IMG");

				Drink drink = new Drink(menu_id, menu_name, menu_price, menu_img);
				drinkList.add(drink);
				...
				// 메뉴의 사진
				JLabel imageIcon = new JLabel(new ImageIcon("./DRINK/" + menu_img));
