...
void Application::Run()
{
    while (1)
    {
        ...
        case 8:      // put data to data file
            WriteDataToFile();
            break;
        case 9: // masterList에 정보 추가
            AddMasterList();
            break;
        case 10: // deptList에 정보 추가
            AddDeptList();
            break;
        case 11: // 학과 검색
            searchFromMasterList();
            break;
        case 0:   // quit
            return;
        default:
...
int Application::GetCommand()
{
    ...
    cout << "\t   9 : Insert Master List" << endl;
    cout << "\t  10 : Insert Dept List" << endl;
    cout << "\t  11 : Search Dept by name" << endl;
    cout << "\t   0 : Quit " << endl;
...
// Master List에 새로운 학생 등록
int Application::AddMasterList() {
    ItemType item;

    cout << "Input master Item : ";
    item.SetRecordFromKB();   //추가할 정보를 입력한다.

    masterList.Add(item);   //리스트에 항목을 추가한다.
    masterList.DisplayOnScreen();

    return 1;
}

// Dept List에 새로운 학생 추가
int Application::AddDeptList() {
    deptType dept;

    cout << "Input dept Item : " << endl;
    dept.SetRecordFromKB(); // 추가할 정보를 입력한다.
    cout << "-------AddDeptList------" << endl;
    dept.DisplayRecordOnScreen();
    deptList.Add(dept); // deptList에 새로운 정보 추가
    cout << "\n-------AddDeptList Add 후-------" << endl;
    deptList.DIsplayOnScreen();
...
deptType* Application::searchDeptByName(string name) {
    deptType dept;
    dept.SetName(name);
    deptList.Get(dept);
    cout << "-------SearchDeptByName---------- " << endl;
    dept.DisplayRecordOnScreen();
    dept.DisplayStudentIDOnScreen();
    return &dept;
}

void Application::printStudentsFromDept(deptType* dept) {
    cout << "--------printStudentsFormDept----------" << endl;
    dept->DisplayStudentIDOnScreen();
    ItemType item;

    for (int i = 0; i < masterList.GetLength(); i++) {
        item.SetId(dept->GetStudent());
        masterList.Get(item);
        item.DisplayRecordOnScreen();
    }
}
// KB로 학과명을 입력받고 학과명과 같은 dept를 찾아서 리턴
// 그 dept의 학번 각각의 정보를 masterlist에서 찾아서 정보 출력
void Application::searchFromMasterList() {
    string name; // KB에서 입력받을 dept name
    deptType* dept;
    cout << "학과명을 입력하시오 : ";
    cin >> name;
    dept = searchDeptByName(name);
    printStudentsFromDept(dept);
