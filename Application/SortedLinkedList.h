...
public:
    ...
    */
    int Get(T& item); // Primary key를 기준으로 데이터를 검색하고 해당 데이터 가져옴
...
void SortedLinkedList<T>::DIsplayOnScreen()
{
    ...
    // 리스트에 node가 하나라도 있다면
        ...
        T item;
        m_pCurPointer = m_pList; // 첫번째 node를 가리키게함.
        while (count < m_nLength)   //리스트의 끝이 아니라면 반복.
        {
            item = m_pList->data;
            cout << item << endl; // 리스트의 data를 출력
            m_pList = m_pList->next; // 다음 node로 이동
            count++;
        }
    }
}
